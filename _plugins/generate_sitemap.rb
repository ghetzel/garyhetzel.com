...
module Jekyll
  ...
  # skip patterns matched in the sitemap_skip_pages config
    def valid_path?(path, site)
      if site.config['sitemap_skip_pages']
        site.config['sitemap_skip_pages'].each do |skiprx|
          if path =~ Regexp.new(skiprx)
            puts "Sitemap: Skipping page #{path}"
            return false
          end
        end
      end
    ...
    def entry(path, date, changefreq, site)
      path.sub!(/textile$/, 'html')
      path.sub!(/\/index\.html$/, '')
      path.sub!(/\.html$/, '')
      return "" if not valid_path?(path, site)
